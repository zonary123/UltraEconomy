              ...
              CommandManager.argument(AdminCommandSupport.KEY_CURRENCY, StringArgumentType.string())
                ...
                .then(
                  CobbleUtilsSuggests.SUGGESTS_PLAYER_OFFLINE_AND_ONLINE
                    .suggestPlayerName(AdminCommandSupport.KEY_PLAYER, List.of("ultraeconomy.admin.deposit"), 2)
                    .executes(context -> {
                      UltraEconomy.runAsync(() -> {
                        ...
                        if (!UltraEconomyApi.deposit(playerUUID, currency.getId(), value)) {
                          CommandFeedback.sendError(context.getSource(), UltraEconomy.lang.getMessageDepositFailed());
                          return;
                        }
                      });
                      return 1;
