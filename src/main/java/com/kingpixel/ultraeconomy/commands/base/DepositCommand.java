 ...
 * Command to deposit money to your own account
 */
public class DepositCommand {
  ...
  private static LiteralArgumentBuilder<ServerCommandSource> get() {
    return CommandManager.literal("deposit")
      .requires(source -> PermissionApi.hasPermission(source, "ultraeconomy.command.deposit", 0))
      .then(
        CommandManager.argument(KEY_AMOUNT, FloatArgumentType.floatArg())
          ...
          .executes(context -> {
            ...
            CommandManager.argument(KEY_CURRENCY, StringArgumentType.string())
              ...
              .executes(context -> {
                ServerPlayerEntity player = context.getSource().getPlayer();
                if (PlayerUtils.hasCooldownCommand(player, "ultraeconomy.command.deposit", UltraEconomy.config.getCommandCooldown()))
                  return 0;
  ...
  public static void run(UUID playerUUID, CommandContext<ServerCommandSource> context, String currencyId) {
    UltraEconomy.runAsync(() -> {
      ...
      if (!success) {
        UltraEconomy.lang.getMessageDepositFailed().sendMessage(
          source.getPlayer(),
          UltraEconomy.lang.getPrefix(),
          false
        );
      ...
      Register.sendMessage(currency, amount, playerUUID, UltraEconomy.lang.getMessageDeposit());
    });
  }
}
