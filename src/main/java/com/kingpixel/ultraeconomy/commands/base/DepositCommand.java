 ...
 * Deposits money to self or to a specified player.
 */
public class DepositCommand {
  private static final String PERMISSION_NODE = "ultraeconomy.command.deposit";
          ...
          .executes(context -> {
            ...
            if (executor == null) {
              CommandFeedback.sendError(context.getSource(),
                "Must specify currency and player when executing from console. Usage: /deposit <amount> <currency> <player>");
              return 0;
            }
          ...
          .then(
            CommandManager.argument(BaseCommandSupport.KEY_CURRENCY, StringArgumentType.string())
              ...
              .executes(context -> {
                ...
                if (executor == null) {
                  CommandFeedback.sendError(context.getSource(),
                    "Must specify a player when executing from console. Usage: /deposit <amount> <currency> <player>");
                  return 0;
                }
              ...
              .then(
                CobbleUtilsSuggests.SUGGESTS_PLAYER_OFFLINE_AND_ONLINE
                  .suggestPlayerName(BaseCommandSupport.KEY_PLAYER, List.of("ultraeconomy.command.deposit.other"), 0)
                  .executes(context -> {
                    var executor = context.getSource().getPlayer();
  ...
  public static void run(UUID playerUUID, CommandContext<ServerCommandSource> context, String currencyId) {
    UltraEconomy.runAsync(() -> {
      ...
      boolean success = UltraEconomyApi.deposit(playerUUID, currencyId, amount);
      if (!success) {
        CommandFeedback.sendError(source, UltraEconomy.lang.getMessageDepositFailed());
      }
    });
  }
}
